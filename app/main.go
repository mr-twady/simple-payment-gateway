...
func main() {
	...
	// to support some degree of resillence
	// Note: sometimes it might take a few seconds for the DB to be ready. Hence, the reason I implemented a simple retry logic
	var db *gorm.DB
	var dbErrx error
	...
	settings := gobreaker.Settings{
		...
		Timeout:     30 * time.Second, // or we could use the configured env value if we want to have a general rule of thumb for timeouts
	}
	...
	transactionService := service.NewTransactionService(transactionRepo, gateways)
